  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">JARVIS</div>
      </div>
...
const handleNextStep = (params = {}) => {
  ...
  // record simulationround number configuration
  if (params.maxRounds) {
  ...
  //Build Resultrouteparameter
  const routeParams = {
    name: 'SimulationRun',
    params: { simulationId: currentSimulationId.value }
  }
  
  // if has customround number , through past query parameter
  if (params.maxRounds) {
    routeParams.query = { maxRounds: params.maxRounds }
  }
  
  // jump turn to Step 3 page
...
/**
 * check closecurrentlyrunsimulation
 * user from Step 3 return to Step 2 time , defaultuser need exit simulation
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // test closesimulation environment
      try {
        ...
        if (closeRes.success) {
          ...
          // ifclosefailed, test strong control stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(t('log.closeSimEnvException', { error: closeErr.message }))
        // ifcloseexception, test strong control stop
        await forceStopSimulation()
      }
    } else {
      // loop not run, but mayprocess also in , check simulationstatus
    ...
    // check loop statusfailed not shadow after continue flow
    console.warn(' check simulationstatusfailed:', err)
  }
}

/**
 * strong control stopsimulation
 */
...
onMounted(async () => {
  ...
  // check closecurrentlyrunsimulation(user from Step 3 return time )
