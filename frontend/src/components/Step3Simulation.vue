          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT: send table comment -->
                ...
                <!-- through fallback: not know type or has content but not was on process -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // from Step2 enter maximum round number
  minutesPerRound: {
    type: Number,
    default: 30 // default each round30 divide
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: not start, 1: running..., 2: already Complete
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // all dynamic (incremental)
const actionIds = ref(new Set()) // at deduplicate dynamic ID set
const scrollContainer = ref(null)

// Computed
// time space order display dynamic ( most new in finally surface , immediately footer)
...
// formatsimulation workflow time space ( root round and each round divide number calculate)
...
// Twittersimulation workflow time space
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Redditsimulation workflow time space
...
// resetallstatus( at new startsimulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling() // stop before may exist in polling
}
...
const doStartSimulation = async () => {
  ...
  // resetallstatus, correctly protect not will to on one times simulation shadow
  resetAllState()
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true, // strong control new start
      enable_graph_memory_update: true // open startgraphupdate
    }
...
// trace each on one times round, at change outputlog
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // divide each round change outputlog
      ...
      // simulation is else already Complete( through past runner_status or Completestatuscheck)
      const isCompleted = data.runner_status === 'Completed' || data.runner_status === 'stopped'
      
      // outside check : ifbackend also from and update runner_status, but alreadyreportComplete
      // through past twitter_Completed and reddit_Completed statuscheck
    ...
    console.warn(' fetch runstatusfailed:', err)
  }
}

// check allenable is else already Complete
const checkPlatformsCompleted = (data) => {
  // ifnodata, return false
  if (!data) return false
  
  // check each Completestatus
  const twitterCompleted = data.twitter_Completed === true
  const redditCompleted = data.reddit_Completed === true
  
  // if few has one Complete, check is else allenable all Complete
  // through past actions_count check is else was enable(if count > 0 or running as true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // ifno was enable, return false
  if (!twitterEnabled && !redditEnabled) return false
  
  // check allenable is else all already Complete
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // using all_actions fetch Complete dynamic list
      const serverActions = res.data.all_actions || []
      
      // incrementaladd new dynamic (deduplicate)
      ...
      // not autoscroll, let user self check see time space
      // new dynamic will in footer
    }
  } catch (err) {
    console.warn(' fetch detailedstatusfailed:', err)
  }
}
