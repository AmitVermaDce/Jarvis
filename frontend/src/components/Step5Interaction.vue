...
const chatHistoryCache = ref({}) // cacheall for speech record : { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
// save before for speech record to cache
...
const selectReportAgentChat = () => {
  ...
  // resume Report Agent for speech record
...
const selectAgent = (agent, idx) => {
  ...
  // resume the Agent for speech record
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // autosave for speech record to cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? ' question ' : ''}: ${msg.content}`)
      .join('\n')
    prompt = ` to below is before for speech : \n${historyContext}\n\n in new question topic is : ${message}`
  }
  ...
  if (res.success && res.data) {
    // positive correct datapath: res.data.result.results is one objectdictionary
    // format: {"twitter_0": {...}, "reddit_0": {...}} or {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // will objectdictionaryconvert as array, fetch reddit reply
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // using reddit reply, second twitter
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // will objectdictionaryconvert as arrayformat
...
/* Left Panel - Report Style ( and Step4Report.vue all one ) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* repeat has order list number - using CSS number let many ol continue number */
.message-text {
...
/* day /questionnaire area domain quotestyle */
