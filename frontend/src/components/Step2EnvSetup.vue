<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 03: generate dual simulation configuration -->
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- simulationround number configuration - only has in configurationGeneration Complete and round number calculate exit from after display -->
...
const props = defineProps({
  simulationId: String, // from parent component enter
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: initialize, 1: generatepersona, 2: generateconfiguration, 3: Complete
...
// logdeduplicate: record on one times outputkeyinfo
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// simulationround number configuration
const useCustomRounds = ref(false) // default using autoconfigurationround number
const customMaxRounds = ref(40)   // defaultrecommend40round

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === 'generateAgentpersona' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'generatesimulation configuration' || newStage === 'generating_config') {
    phase.value = 2
    // enter enter configurationgenerate segment , startpollingconfiguration
    if (!configTimer) {
      addLog(t('log.startGeneratingConfig'))
      startConfigPolling()
    }
  } else if (newStage === 'preparingsimulation' || newStage === 'copying_scripts') {
    phase.value = 2 // belong at configuration segment
  }
})

// from configuration in calculateauto-generateround number ( not using encodingdefault value )
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // configuration not generate time return null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // configurationdata not Complete time return null
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // correctly protect maximum round number not small at 40(recommend value ), avoid exempt dynamic entries rangeexception
...
// root agent_id fetch for should username
...
// calculateallpersonaassociated speech topic number
...
// processstartsimulationbuttonclick
const handleStartSimulation = () => {
  //Build Resultto parent componentparameter
  const params = {}
  
  if (useCustomRounds.value) {
    // usercustomround number , max_rounds parameter
    params.maxRounds = customMaxRounds.value
    addLog(t('log.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // userselect protect hold auto-generateround number , not max_rounds parameter
...
const startPrepareSimulation = async () => {
  ...
  // mark # one stepComplete, start # two step
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // immediately settings period Agent number ( from prepareAPI return value fetch )
      ...
      startPolling()
      // start actual time fetch Profiles
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // parse segment infooutputdetailedlog
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage_name || ''
        
        // outputdetailedprogresslog( avoid exempt duplicate)
        ...
        // from message in extract segment
        const match = data.message.match(/\[(\d+)\/(\d+)\]\s*([^:]+)/)
        if (match) {
          currentStage.value = match[3].trim()
        }
        // outputmessagelog( avoid exempt duplicate)
    ...
    console.warn('pollingstatusfailed:', err)
  }
}
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // only has API return valid value time update, avoid exempt override already has valid value
      ...
      // output Profile Generation Progresslog( only number amount change time )
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // outputconfigurationgenerate segment log( avoid exempt duplicate)
      ...
      // ifconfiguration already generate
      if (data.config_generated && data.config) {
        simulationConfig.value = data.config
        addLog(t('log.configComplete'))

        // displaydetailedconfigurationsummary
        if (data.summary) {
        ...
        // display time space configurationdetails
...
const loadPreparedData = async () => {
  ...
  // finally fetch one times Profiles
  await fetchProfilesRealtime()
  addLog(t('log.loadedAgentProfiles', { count: profiles.value.length }))

  // fetch configuration( using actual time API)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // configurationnot yetgenerate, startpolling
...
/* simulationround number configurationstyle */
