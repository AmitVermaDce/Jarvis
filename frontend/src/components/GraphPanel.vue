...
const showEdgeLabels = ref(true) // defaultdisplaylabel
const expandedSelfLoops = ref(new Set()) // expand self loop items
const showSimulationFinishedHint = ref(false) // simulationend after hint
const wasSimulating = ref(false) // trace before is else in simulation in
...
// listen isSimulating change , simulationend
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // from simulation in change as non-simulationstatus, displayendhint
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// switch switch self loop items expand/collapsestatus
...
// calculateentity types at chart
...
const renderGraph = () => {
  ...
  // processdata, calculate same one for node space number amount and index
  const edgePairCount = {}
  const selfLoopEdges = {} // node divide group self loop
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // statistics each for node space number amount , collect self loop
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // self loop - collect to array in
  ...
  // record before process to each for node # several entries
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // already process self loop node
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // self loop - eachnode only add one entries merge self loop
      if (processedSelfLoopNodes.has(e.source_node_uuid)) {
        return // already process past , skip
      }
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // storageall self loop detailedinfo
        }
      })
      return
    }
    ...
    // check toward is else and standard toward one ( source UUID < item mark UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // calculate rate : many entries time divide scattered open , entries as line
    let curvature = 0
    if (totalCount > 1) {
      // uniformdistribution rate , correctly protect distinguish
      // rate range root number amount increase , many rate range large
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // if toward and standard toward , turn rate
      // this pattern correctly protect all in same one reference system below distribution, not will because toward different and
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - root number amount dynamic adjustnodespacing
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // root this for node space number amount dynamic adjust
      // basic 150, each many one entries increase 40
    ...
    // add toward in lead , letdependentnode set to in area domain
  ...
  const getLinkPath = (d) => {
    ...
    // self loop
    if (d.isSelfLoop) {
      // self loop : draw one arc from node exit send again return
      const loopRadius = 30
      // from noderight exit send , bypass one circle return from
      const x1 = sx + 8 // point offset
      const y1 = sy - 4
      const x2 = sx + 8 // end point offset
      const y2 = sy + 4
      // using arcdraw self loop (sweep-flag=1 time )
    ...
    // calculatecurve control control point - root number amount and dynamic adjust
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // straight at line toward offset, root calculate, protect certificate curvevisible
    // many , offset amount large
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // basic25%, each many one entries increase 5%
  ...
  // calculatecurve in point ( at label fixed )
  const getLinkMidpoint = (d) => {
    ...
    // self loop
    if (d.isSelfLoop) {
      // self loop labelposition: noderight
      return { x: sx + 70, y: sy }
    }
    ...
    // two times curve in point t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // two times curve type B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Link labels background ( color background using text char more )
  ...
  // savequote provide external control control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // only record position, not simulation(distinguishclick and drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // is else true positive startdrag( dynamic exceed past threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // first times to true positive drag, simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // only has true positive drag past let simulationstop
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // updatelabelposition( no rotate, display more )
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // removerotate, protect hold
    })
