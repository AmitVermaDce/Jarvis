    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - in Complete after display -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (contentGeneration Complete, but whole sectionmay also Complete) -->
                  ...
                  <!-- Section Complete (sectionGeneration Complete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- is most end answer time , display special hint -->
...
const toggleRawResult = (timestamp, event) => {
  // savebutton for at port position
  ...
  // waiting DOM update after , adjustscrollposition to protect hold button in same position
...
const toggleSectionCollapse = (idx) => {
  // only has already Completesection can collapse
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // icon - table insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // icon - table all search
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // usericon - table for speech
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // icon - table fast speed
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // chart table icon - table statistics
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // databaseicon - table entity
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // extractanalyze question topic
    const queryMatch = text.match(/analyze question topic :\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // extractpredictionscenario
    const reqMatch = text.match(/predictionscenario:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // extractstatisticsdata - match"relatedprediction actual : X entries "format
    const factMatch = text.match(/relatedprediction actual :\s*(\d+)/)
    const entityMatch = text.match(/ and entity:\s*(\d+)/)
    const relMatch = text.match(/relation chain :\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // extract sub question topic - Completeextract, not limit number amount
    const subQSection = text.match(/### analyze sub question topic \n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // extractkey actual - Completeextract, not limit number amount
    const factsSection = text.match(/### [key actual ][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // extractcoreentity - Completeextract, containsummary and related actual number
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/related actual :\s*(\d+)/)
        return {
    ...
    // extractrelation chain - Completeextract, not limit number amount
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const nodesMatch = text.match(/node number :\s*(\d+)/)
    const edgesMatch = text.match(/ number :\s*(\d+)/)
    const activeMatch = text.match(/ before valid actual :\s*(\d+)/)
    const histMatch = text.match(/\/expired actual :\s*(\d+)/)
    ...
    // extract before valid actual - Completeextract, not limit number amount
    const activeSection = text.match(/### [ before valid actual ][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // remove number and lead number
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // extract/expired actual - Completeextract, not limit number amount
    const histSection = text.match(/### [\/expired actual ][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // extract and entity - Completeextract, not limit number amount
    const entitySection = text.match(/### [ and entity]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // extractinterviewtheme
    const topicMatch = text.match(/\*\*interviewtheme:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // extractinterview number ( if "5 / 9 simulationAgent")
    const countMatch = text.match(/\*\*interview number :\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // extractinterviewobjectselect
    const reasonMatch = text.match(/### interviewobjectselect\n([\s\S]*?)(?=\n---\n|\n### interview actual record )/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // format1: number char . ** name char (index=X)**:
        // if : 1. **_345(index=1)**: as large ...
        ...
        // format2: - select name char (index X):
        // if : - select long _601(index 0): as long group table ...
        if (!headerMatch) {
        ...
        // format3: - ** name char (index X)**:
        // if : - ** long _601(index 0)**: as long group table ...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^ not |^ on |^ most end select/)) {
          // continue line (exclude tail paragraph)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // extract simple
      const bioMatch = block.match(/_ simple :\s*([\s\S]*?)_\n/)
      if (bioMatch) {
        interview.bio = bioMatch[1].trim().replace(/\.\.\.$/, '...')
      }
      
      // extract question topic list
      const qMatch = block.match(/\*\*Q:\*\*\s*([\s\S]*?)(?=\n\n\*\*A:\*\*|\*\*A:\*\*)/)
      if (qMatch) {
        const qText = qMatch[1].trim()
        // number char number divide question topic
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // if # one question topic before surface has "1.", need to special process
      ...
      // extract return answer - divide Twitter and Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*keyintroduction|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // separateTwitter and Reddit return answer
        const twitterMatch = answerText.match(/[Twitter return answer ]\n?([\s\S]*?)(?=[Reddit return answer ]|$)/)
        const redditMatch = answerText.match(/[Reddit return answer ]\n?([\s\S]*?)$/)
        ...
        // fallbacklogic(compatible old format: only has one mark)
        if (!twitterMatch && redditMatch) {
          // only has Reddit return answer , only in non- text time copy as defaultdisplay
          if (interview.redditAnswer && interview.redditAnswer !== '( the not reply)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '( the not reply)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // no divide mark( old format), whole body as return answer
          interview.twitterAnswer = answerText
        }
      }
      
      // extractkeyintroduction(compatible many types lead number format)
      const quotesMatch = block.match(/\*\*keyintroduction:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // match > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // fallback: match > "text" or > \u201Ctext\u201D( in Chinese lead number )
        if (!quoteMatches) {
    ...
    const summaryMatch = text.match(/### interviewsummary and coreviewpoint\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/ find to \s*(\d+)\s* entries /)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // extractrelated actual - Completeextract, not limit number amount
    const factsSection = text.match(/### related actual :\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // test extractinfo(if has )
    const edgesSection = text.match(/### related:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // test extractnodeinfo(if has )
    const nodesSection = text.match(/### relatednode:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // test extract time space info [time - time]
                  (() => {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // as each question topic - return answer for guard independentselectstatus
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // fetch certain question topic before select
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // settings certain question topic select
    ...
    // check is else as text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '( the not reply)' || t === '( the not reply)' || t === '[ no reply]'
    }

    // test question topic number divide return answer
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // support types number format:
      // 1. " question topic X: " or " question topic X:" ( in Chinese format, backend new format)
      // 2. "1. " or "\n1. " ( number char + point , old formatcompatible)
      let matches = []
      let match

      // test " question topic X: " format
      const cnPattern = /(?:^|[\r\n]+) question topic (\d+)[: :]\s*/g
      ...
      // ifmatch to , fallback to " number char ." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // ifno find to number or only find to one , return whole body
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^ question topic \d+[: :]\s*/, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
        return [cleaned || answerText]
      }

      // number extract each partial
      const parts = []
    ...
    // fetch certain question topic for should return answer
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // divide success and indexvalid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // divide failed: # one question topic return Complete return answer , its remaining return empty
      return qIdx === 0 ? answer : ''
    }
    
    // check certain question topic is else has dual return answer (filter text)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // all has actual return answer (non- text) and content different
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        props.result.interviews[activeIndex.value]?.selectionReason && h('div', { class: 'selection-reason' }, [
          h('div', { class: 'reason-label' }, 'select'),
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // dual switch switch button( only in has actual dual return answer time display)
                  ...
                  // Expand/Collapse Button( text not display)
...
// before activestep( at topdisplay)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // find to before active step
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // ifno active, return finally one done step
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // otherwise return # one step
...
const renderMarkdown = (content) => {
  ...
  // to open head two level title(## xxx), becausesectiontitle already in outside layer display
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // process code block
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // process line within code
  ...
  // package install no order list
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // package install has order list
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // cleanuplist items space all empty
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // cleanupliststartlabel after empty
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // cleanuplistendlabel before empty
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')
  
  // processbold and italic
  ...
  // process switch line - empty line change paragraph divide , switch line change <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // package install in paragraph in
  html = '<p class="md-p">' + html + '</p>'
  
  // cleanup empty paragraph
  html = html.replace(/<p class="md-p"><\/p>/g, '')
  html = html.replace(/<p class="md-p">(<h[2-5])/g, '$1')
  html = html.replace(/(<\/h[2-5]>)<\/p>/g, '$1')
  html = html.replace(/<p class="md-p">(<ul|<ol|<blockquote|<pre|<hr)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>|<\/pre>)<\/p>/g, '$1')
  // cleanup block level before after <br> label
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // cleanup <p><br> tight block level ( many remaining empty line guide )
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // cleanup continue <br> label
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // cleanup block level after tight paragraphstartlabel before <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // repeat non- continue has order list number : items <ol> was paragraphcontent open time , protect hold number increase
...
const getLogLevelClass = (log) => {
  ...
  // INFO using defaultcolor, not mark as success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_Complete - sectionGeneration Complete
          if (log.action === 'section_Complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // autoexpandgeneratesection
          ...
          if (log.action === 'report_Complete') {
            ...
            currentSectionIndex.value = null // correctly protect clear loading status
            emit('update-status', 'Completed')
            stopPolling()
            // scrolllogic unified in loopend after nextTick in process
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // iftask already Complete, scroll to top; otherwisescroll to footer most new log
            if (isComplete.value) {
...
// extract most end answer content - from LLM response in extractsectioncontent
const extractFinalContent = (response) => {
  if (!response) return null
  
  // test extract <final_answer> label within content
  ...
  // test find Final Answer: after surface content(support many types format)
  ...
  // test find most end answer : after surface content
  const chineseFinalMatch = response.match(/ most end answer [:: ]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // if to ## or # or > open head , may is directly markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // ifcontent long and containmarkdownformat, test remove think past process after return
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // remove Thought: open head think past process
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* deep color , not status change */
  font-weight: 500;
}
...
/* Console Logs - and Step3Simulation.vue protect hold one */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
