"""Zep Graph divide read fetch tool.

Zep node/edge listAPI using UUID cursor divide ,
 model block install autologic( contain retry), for calltransparent return completelist.
"""
...
logger = get_logger('jarvis.zep_paging')
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """request, failed time point number avoid retry. only retry network network /IOclasserror. """
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """ divide fetch graphnode, most many return max_items entries (default 2000). each request self with retry. """
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """ divide fetch graphall, return completelist. each request self with retry. """
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
